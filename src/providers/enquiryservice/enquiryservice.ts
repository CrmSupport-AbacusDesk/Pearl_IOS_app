...
export class EnquiryserviceProvider {
  ...
  public getCustomerType(){
    return new Promise((resolve, reject) => {
        ...
        this.http.get(this.constant.server_url+'login/getCustomerType',{headers: header}).map(res=>res.json())
        .subscribe(res=>{
          console.log(res);
          resolve(res);
        }, (err) => {
  ...
  public getCity1(val){
    return new Promise((resolve, reject) => {
        let header = new Headers();
        header.append('Content-Type', 'application/json');
        this.http.post(this.constant.server_url+'enquiry/get_city',JSON.stringify(val),{headers: header}).map(res=>res.json())
        .subscribe(res=>{
          console.log(res);
          resolve(res);
        }, (err) => {
  ...
  public get_pincode_city_name(val){
    return new Promise((resolve, reject) => {
        let header = new Headers();
        header.append('Content-Type', 'application/json');
        this.http.post(this.constant.server_url+'enquiry/pincode_city_name',JSON.stringify(val),{headers: header}).map(res=>res.json())
        .subscribe(res=>{
          console.log(res);
          resolve(res);
        }, (err) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        this.storage.get('token').then((token) => {
            ...
            if(typeof(token) !== 'undefined' && token) {
                ...
                this.http.post(this.constant.server_url+'product/submit_enquiry',JSON.stringify(enquiry),{headers: header}).map(res=>res.json())
                .subscribe(res=>{
                  console.log(res);
                  resolve(res);
                }, (err) => {
